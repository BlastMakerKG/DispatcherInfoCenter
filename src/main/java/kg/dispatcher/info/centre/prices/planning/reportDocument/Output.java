...
import kg.dispatcher.info.centre.prices.planning.functions.*;
...
public class Output {
    ...
    public List<Excavator> waste(int day){
        ...
        for (Excavator perDayExcavator : objects) {
            ...
            for (Truck str : perDayExcavator.getTrucks()) {
                ...
                Truck comeTruck = get_come(new Timestamp(truckTripsRepository.findAllDateTime().get(day - 1).getTime()), str.getType_truck());
    ...
    public List<Waste> month(){
        ...
        int days = truckTripsRepository.findAllDateTime().size();
...
//            List<String> exsByName = truckTripsRespository.AllNameExByDay(new Timestamp(truckTripsRespository.findAllDateTime().get(i).getTime()));
            ...
            List<Integer> nums = truckTripsRepository.findNums(new Timestamp(truckTripsRepository.findAllDateTime().get(i).getTime()));
            ...
            for(Integer num : nums){
                ...
                List<TruckTripsDTO> truckTripsDTOS = truckTripsRepository.findByTimeOfComeLoadingaAndNumSamosval(new Timestamp(truckTripsRepository.findAllDateTime().get(i).getTime()), num);
    ...
    private Truck get_come(Timestamp date, String truckName){
        List<TruckTripsDTO> truckTripsDTOS = truckTripsRepository.getObjectsByDateAndByTypeTruck(date, truckName);
