...
import kg.dispatcher.info.centre.prices.planning.model.PerReice;
...
public class OriginalData {
    ...
    @Autowired
    TruckTripsRepository truckTripsRepository;
    ...
    public HashMap<Integer, Month> month(String type){
        ...
        for(Date date : truckTripsRepository.findAllDateTime()){
        ...
        for(Timestamp date : dates){
            ...
            if(type.equals("")) {
                truckTripsDTOS = truckTripsRepository.findByTimeOfComeLoading(date);
            }else {
                truckTripsDTOS = truckTripsRepository.findObjectByTypeOfWorkAndTime(type, date);
            }
            ...
            for(String str : temp_trucks){
                int count = truckTripsRepository.findCountSamosvalbyDateAndType(date, str).size();
            ...
            for(String ex : temp_exs){
                int count = truckTripsRepository.findCountEx(date, ex).size();
                exs.add(ex+" - "+count);
            }
            ...
            month.put(days, Month.builder()
                    ...
                    .gas(gas)
                    .quantityReise(truckTripsRepository.findbyReise(date).size())
    ...
    public List<Truck> perDayByTruck(int days){
        ...
        List<Integer> nums = truckTripsRepository.findNums(new Timestamp(truckTripsRepository.findAllDateTime().get(days-1).getTime()));

        for(Integer num : nums) {
            List<TruckTripsDTO> orderByNum = truckTripsRepository.findByTimeOfComeLoadingaAndNumSamosval(new Timestamp(truckTripsRepository.findAllDateTime().get(days - 1).getTime()), num);
    ...
    public List<Excavator> perDayByExes(int day){
        ...
        List<String> nums = truckTripsRepository.AllNameExByDay(new Timestamp(truckTripsRepository.findAllDateTime().get(day-1).getTime()));

        for(String ex : nums) {
            List<TruckTripsDTO> orderByEx = truckTripsRepository.findAllObjectByEx(new Timestamp(truckTripsRepository.findAllDateTime().get(day - 1).getTime()), ex);
            ...
            for(TruckTripsDTO ob : orderByEx){
                ...
                if(!trucks.contains(ob.getTruck().getTruckTypeDTO().getName())) {
                    ...
                    tr_temp.add(Truck.builder()
                            ...
                            .count_truck(truckTripsRepository.findQuantitySamByEx(new Timestamp(truckTripsRepository.findAllDateTime().get(day - 1).getTime()), ex, ob.getTruck().getTruckTypeDTO().getName()).size())
    ...
    public List<PerReice> perReice(int days, int num){
        ...
        List<TruckTripsDTO> truckTripsDTOS = truckTripsRepository.findByTimeOfComeLoadingaAndNumSamosval(new Timestamp(truckTripsRepository.findAllDateTime().get(days - 1).getTime()), num);
